...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("escalates a successful assignment recovery that still leaves todo, without retrying forever", async () => {
    ...
    // This release wakes the same owner for the source-scoped, status-only
    // escalation. That is not another assignment-recovery execution retry.
    expect(agentRuns).toHaveLength(2);
    expect(agentRuns.filter((row) => row.contextSnapshot?.retryReason === "assignment_recovery")).toHaveLength(1);
    expect(agentRuns.filter((row) => row.contextSnapshot?.wakeReason === "source_scoped_recovery_action"))
      .toEqual([expect.objectContaining({ contextSnapshot: expect.objectContaining({
        sourceIssueId: issueId, recoveryIntent: "status_only", allowDeliverableWork: false,
      }) })]);
